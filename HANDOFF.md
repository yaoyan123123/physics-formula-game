...
1. ✅ **[已完成]** 湖南合格考版已上线（2026-07-19，含 xk 榜正确率校验修复）。
2. 🟡 **[技术债·已контейн]** 部署仓库含 Codex 的 `.worktrees/`(100M, feat分支, 0未提交)、docs/、tests/、node_modules。**泄露已三层兜住**：`.assetsignore` + 暂存 rsync 部署法(§2) + `.gitignore`。docs/tests 是正当跟踪内容(tests=回归套件有价值,勿删)。唯一可优化=删 100M worktree 回收空间——但那是 Codex 工作区，**需与 Codex 协调后再删**(`git worktree remove` 可逆,分支保留)，CC 不擅自删另一条流水线的工作区。
3. **[可选增长·待用户方向]** PWA 离线(sw.js/manifest/icon 已备,按用户要求关闭中)、变现钩子(抖音号引流位)、访问统计观察、湖南版跑一段看数据是否需微调。

**核心产品状态：完成且线上健康。** 后续只剩可选增长(需用户定方向)与一项需协调的空间回收。
